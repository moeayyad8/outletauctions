...
import { Package, Camera, X, Plus, Printer, Trash2, Send, ScanLine, Archive, ImagePlus } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto p-4 pb-24 space-y-6">
        ...
        <Tabs defaultValue="batch" className="w-full">
          <TabsList className="grid w-full grid-cols-2 h-12 rounded-xl">
            <TabsTrigger value="batch" className="rounded-lg gap-2 data-[state=active]:shadow-sm">
              <Package className="w-4 h-4" />
              Batch
              {batch.length > 0 && (
                <span className="ml-1 px-2 py-0.5 bg-primary/20 text-primary text-xs rounded-full font-medium">
                  {batch.length}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="inventory" className="rounded-lg gap-2 data-[state=active]:shadow-sm">
          ...
          <TabsContent value="inventory" className="mt-6 space-y-2">
            {auctions.length === 0 ? (
              <div className="text-center py-16 px-4">
                <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mx-auto mb-4">
                ...
                <p className="text-muted-foreground text-sm">
                  Scanned items will appear here after sending
                </p>
              </div>
            ) : (
